...
QString User::serializeUserFields() const
{
    ...
    QString safeFullName = PipeEscape::escape(fullName);
    QString result = "";
...
int User::deserializeUserFields(const QStringList &tokens, int startIndex)
{
    ...
    id = tokens.at(index).toULongLong(); index++;
    createdAt = QDateTime::fromString(tokens.at(index), Qt::ISODate); index++;
    updatedAt = QDateTime::fromString(tokens.at(index), Qt::ISODate); index++;
    role = static_cast<Role>(tokens.at(index).toInt()); index++;

    fullName = tokens.at(index); index++;
    fullName = PipeEscape::unescape(fullName);
    username = tokens.at(index); index++;
    email = tokens.at(index); index++;
    passwordHash = tokens.at(index); index++;
    walletId = tokens.at(index).toULongLong(); index++;
    isBlocked = (tokens.at(index) == "1"); index++;

    reviewIds.clear();
    QString reviewsToken = tokens.at(index); index++;
    ...
    QString notifToken = tokens.at(index); index++;
